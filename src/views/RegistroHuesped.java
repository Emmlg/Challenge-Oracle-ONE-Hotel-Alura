...
public class RegistroHuesped extends JFrame {
	...
	private ValidarString validacionString = new ValidarString();
	private HuespedController HuespedContoller = new HuespedController();
	...
	public RegistroHuesped() {
		...
		txtNacionalidad.setFont(new Font("Dialog", Font.PLAIN, 16));
		...
		txtTelefono.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				txtNreserva.setText(""+reservaContoller.getIdReserva());
			}
		});
		...
		lblTitulo.setBounds(514, 55, 326, 42);
		...
		txtNreserva.setForeground(new Color(0, 0, 0));
		...
		labelGuardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Boolean validarName =
					validacionString.isLetter(txtNombre.getText()) &&
					validacionString.isLetter(txtApellido.getText());				
						
			    Boolean validarFech = txtFechaN.getDate() != null;
			    boolean validarPhone = validacionString.isNumeric(txtTelefono.getText());
	            
			    if(validarFech && validarName && validarPhone) {
			    	

					var reghuesped = new Huesped( //creacion de objeto de tipo huesped
							
							txtNombre.getText(),
							txtApellido.getText(),
							validacionString.getDate(txtFechaN),
							txtNacionalidad.getSelectedItem().toString(),
							txtTelefono.getText(),
							Integer.valueOf(txtNreserva.getText())
							
							);
					HuespedContoller.guardarHuesped(reghuesped);	
					this.limpiarHuesped();
					
					// regreso a menu login
					MenuUsuario regresarMenuUsuario =new MenuUsuario();
					regresarMenuUsuario.setVisible(true);
					dispose();
			    ...
			    (null, "¡ verifique sus datos !","¡ Aviso !",JOptionPane.WARNING_MESSAGE);
			    }
			...
			private void limpiarHuesped() {
				...
				txtApellido.setText("");
				txtFechaN = null;
				txtNacionalidad.setSelectedItem(0);
				txtNreserva.setText("");
				txtTelefono.setText("");
			}
